...
class GraphRunner:
    ...
    def can_use_cuda_graph(self, batch: Batch) -> bool:
        # Captured decode graphs assume exactly 1 query position per request; a
        # speculative-decoding verify batch scores >1 positions for some requests, so
        # it must always fall back to the eager (ragged-attention-capable) path.
        return (
            batch.is_decode
            and batch.size <= self.max_graph_bs
            and not batch.has_speculative_rows
        )
